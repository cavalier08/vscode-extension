...
export function activate(context: vscode.ExtensionContext) {
  ...
  provider.registerEditListener(context);
}

class FileRegexManagerViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'fileRegexManagerView';
  ...
  private async executeSingleCommand(commandText: string): Promise<string> {
  ...
  private parseCommand(text: string): { action: string; args: string } | null {
  ...
  private async createFile(args: string, workspaceFolder: vscode.WorkspaceFolder): Promise<string> {
  ...
  private async deleteFile(args: string, workspaceFolder: vscode.WorkspaceFolder): Promise<string> {
  ...
  private async modifyFile(args: string, workspaceFolder: vscode.WorkspaceFolder): Promise<string> {
  ...
  private async listFiles(args: string, workspaceFolder: vscode.WorkspaceFolder): Promise<string> {
...
<body>
    <div class="container">
        ...
        <div class="command-area">
            ...
            <div class="button-group">
                ...
                <button onclick="copyCommandDoc()">Copy All</button>
            </div>
        </div>
        ...
        <!-- Edit log UI -->
        <div id="editLog" style="margin-top:20px; padding:10px; background:var(--vscode-editor-background); border:1px solid var(--vscode-panel-border); border-radius:4px; font-size:12px; max-height:200px; overflow:auto;"></div>

        <!-- Document-style command log -->
        <div id="commandDoc" style="margin-top:16px; padding:12px; background:var(--vscode-editor-background); border:1px solid var(--vscode-panel-border); border-radius:4px; font-size:13px; min-height:100px; max-height:300px; overflow:auto; white-space:pre-wrap; cursor:text;">
    ...
    <script>
        ...
        const commandDoc = document.getElementById('commandDoc');
        ...
        function executeCommands() {
            ...
            // Append commands to the document area
            commands.forEach(cmd => appendCommandToDoc(cmd));
        }

        function appendCommandToDoc(cmd) {
          const entry = document.createElement('div');
          entry.textContent = \`[${new Date().toLocaleTimeString()}] \${cmd}\`;
          entry.style.marginBottom = '2px';
          commandDoc.appendChild(entry);
          // Scroll to bottom
          commandDoc.scrollTop = commandDoc.scrollHeight;
        }

        function copyCommandDoc() {
          const range = document.createRange();
          range.selectNodeContents(commandDoc);
          const selection = window.getSelection();
          selection.removeAllRanges();
          selection.addRange(range);
          try {
            document.execCommand('copy');
            showStatus('Command log copied!', 'success');
          } catch (e) {
        ...
        // Listen for file edit events from extension
        window.addEventListener('message', event => {
          const message = event.data;
          if (message.type === 'fileEdit') {
            const { filePath, edits } = message;
            edits.forEach(edit => {
              const lineInfo = edit.start === edit.end ? \`Line \${edit.start}\` : \`Lines \${edit.start}-\${edit.end}\`;
              const logEntry = document.createElement('div');
              logEntry.textContent = \`[\${new Date().toLocaleTimeString()}] \${filePath}: \${lineInfo}\`;
              logEntry.style.marginBottom = '2px';
              editLog.appendChild(logEntry);
              // Scroll to bottom
